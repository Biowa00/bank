...
import { markAllNotificationsRead } from "@/app/[lang]/dashboard/actions";
...
export default async function NotificationsPage() {
  ...
  const t = (await getRequestDictionary()).dashboard.notifications;
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        {hasUnread && (
          <form action={markAllNotificationsRead}>
            <button type="submit" className="btn-outline text-sm">{t.markAllRead}</button>
          </form>
        )}
      </div>
      ...
      <div className="card divide-y divide-black/5 p-2">
        {notifs && notifs.length > 0 ? (
            ...
            <div key={n.id} className="flex gap-3 px-4 py-4">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="shrink-0 text-xs text-ink/40">{formatDate(n.created_at, locale)}</span>
                </div>
          ...
          <p className="py-10 text-center text-sm text-ink/50">{t.empty}</p>
        )}
      </div>
    </div>
  );
