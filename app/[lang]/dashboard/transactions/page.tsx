...
const typeKeys = ["all", "deposit", "transfer", "withdrawal"] as const;
const statusKeys = ["all", "success", "pending", "rejected", "blocked"] as const;
...
export default async function TransactionsPage({
  ...
  const t = (await getRequestDictionary()).dashboard.transactions;
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap gap-2">
          {typeKeys.map((key) => (
            <Link key={key} href={{ query: { type: key, status } }} className={chip(type === key)}>
              {t.typeFilters[key]}
            </Link>
          ))}
        </div>
        <div className="flex flex-wrap gap-2">
          {statusKeys.map((key) => (
            <Link key={key} href={{ query: { type, status: key } }} className={chip(status === key)}>
              {t.statusFilters[key]}
            </Link>
          ))}
        </div>
